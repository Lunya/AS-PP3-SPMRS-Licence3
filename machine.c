...
void reconstruct_attributes(struct machine * m, struct attributes * tail){
    ...
    if(m->stack!=NULL && m->stack->top->type==TREECOMPATT){
        ...
        child = m->stack->top->item->tree_att->child;
        ...
        m->closure = mk_closure(child,env);
        compute(m);
    }
    else{
...
void pop_forestcomptail(struct machine * m){
    ...
    case INTEGER:
        ...
        break;
    default:
        fprintf(stderr,"Erreur de typage: une forêt ne peut être construite qu'Ã  partir d'une forêt, d'un arbre, d'un mot ou d'un entier");
        exit(1);
    }
...
void pop_treecompforest(struct machine * m){
    ...
    case INTEGER:
        ...
        break;
    default:
        fprintf(stderr,"Erreur de typage: un forêt ne peut être construite qu'Ã  partir d'une forêt, d'un arbre, d'un mot ou d'un entier");
        exit(1);
    }
...
void on_import(struct machine * m){
    assert(m!=NULL);
    fprintf(stderr,
            "Import de fichier Ã  implémenter");
    exit(1);    
}
...
void on_tree(struct machine * m){
    ...
    if(m->closure->value->node->tree->is_value){
        ...
        if(m->closure->value->node->tree->child == NULL ||
           (m->closure->value->node->tree->child->type==FOREST &&
            m->closure->value->node->tree->child->node->forest->is_value)){
            ...
            m->closure = mk_closure(m->closure->value->node->tree->child,
                                    m->closure->env);
            compute(m);
        }
    }
        ...
        tree_att->child=m->closure->value->node->tree->child;
